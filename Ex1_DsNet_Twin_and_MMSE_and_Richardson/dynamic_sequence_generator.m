...
DSS_xx = zeros(l,r); % DSS_xx represents the DSS form for gradient.
...
for i=1:l
    for j=1:r
       ...
       b1(i,j) = and_door(DSS_y(i,j),DSS_xx(i,j));%stochastic multiplication implemented by AND gates and XOR gates
       a1_sign(i,j) = xor_door(DSS_x_sign(i,j),DSS_xx_sign(i,j));
       b1_sign(i,j) = xor_door(DSS_y_sign(i,j),DSS_xx_sign(i,j));
    end
end

%% stochastic integrator to optimize the weight matrix following the truth table
for i=1:l
    for j=1:r      
