%%%%%%%%%%%%%%%%%%%%%%%%%  Notice  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%"data_generator.m" is the first main function of our code, which aims to
...
%% H generation
for seed = 1:1:100 %generating 100 channels and data sets randomly

[H,AT,AR,aoa,aod] = channel_generation(Nt,Nr,seed,N_c,N_ray);

%% input data generation
x = y * H; %since y is the constellation point, thus the multiplication can be achieved by MUX wiht low-complexity as illustrated in Fig.10(a) of our paper.
training_data = x;
...
function [H,AT,AR,aoa,aod] = channel_generation(N_t,N_r,seed,N_c,N_ray)%SCM channel£¬N_c*N_ray>4,rank>4£¬
rng(seed)                                                   % setting random seed
%-----------AOA
E_aoa = 2*pi* rand(N_c,1);                               %the mean of cluster having uniform distribution within£¨0,2*pi£©
sigma_aoa = 10*pi/180;                                    %10¡ãto rad£¬standard deviation
b = sigma_aoa/sqrt(2);                                    
a = rand(N_c,N_ray)-0.5;                                  %random series between(-0.5,0.5) having uniform distribution
aoa = repmat(E_aoa,1,N_ray)-b*sign(a).*log(1-2*abs(a));   %Generate a random sequence that conforms to the Laplacian distribution (each column represents the Angle of each cluster)  
aoa = sin(aoa);
%-----------AOD
